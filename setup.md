...
> 쿠버네티스와 네트워크 간의 인터페이스
> - calico를 사용한다.(네트워크를 제공하는 여러 솔루션중 하나)
- 리눅스 컨테이너 간의 네트워크 연결 및 관리를 구성하기 위한 표준 인터페이스.
- 주로 쿠버네티스(Kubernetes) 환경에서 파드(Pod)에 IP를 할당하고 네트워크 환경을 제어하는 핵심 기술로 사용
